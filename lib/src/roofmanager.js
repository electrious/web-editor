 ...
 * @param ns
 */
function getRoofChange(ns) {
    return core_1.mergeArray(pluck_1.default('roofForFlatten', ns));
}
/**
...
function createRoofManager(meshData, defRoofs) {
    ...
    var roofs = helper_1.unwrap(core_1.map(roofsToRender, roofsData));
    var roofsLst = core_1.map(values_1.default, roofs);
    // create roof node for each roof
    var nodes = core_1.multicast(core_1.map(map_1.default(mkNode), roofsLst));
    ...
    var updatedRoofOp = core_1.multicast(core_1.switchLatest(core_1.map(getRoofUpdate, nodes)));
    // stream of delete roof operations
    var deleteRoofOp = core_1.multicast(core_1.switchLatest(core_1.map(getRoofDelete, nodes)));
    var roofChangeOp = core_1.switchLatest(core_1.map(getRoofChange, nodes));
    ...
    var newRoofs = core_1.multicast(core_1.map(roofsForFlatten, roofsData));
    ...
    var recognizer = roofrecognizer_1.createRoofRecognizer(meshData.wrapper, core_1.map(values_1.default, newRoofs), meshData.mesh.mouseMoveEvent);
    ...
    return {
        roofWrapper: wrapper,
        roofOps: core_1.multicast(core_1.mergeArray([addRoofOp, deleteRoofOp, updatedRoofOp])),

...
function createEditor(width, height, elem) {
    ...
    var loadHouseFunc = function (dataServerUrl, leadId, roofs, roofEdited) {
        var loadRoofs = function (md) {
            ...
            disposables.push(mgr.editedRoofs.run(sink_1.mkSink(roofEdited), helper_1.defScheduler()));
        ...
        var disp = house_1.loadHouse(dataServerUrl, leadId).run(sink_1.mkSink(f), helper_1.defScheduler());
